    ...
    .map((exam, index) => ({
      ...
      examenes: {
        ...
        validado: exam.examenes.validado
          ? expandedGroups.value.has(index)
            ? exam.examenes.validado
            : exam.examenes.validado.slice(0, 3)
          : [],
      },
    }))
    .filter(
      (exam) => exam.examenes.validado && exam.examenes.validado.length > 0
    );
});
...
const pendingExams = computed(() => {
  ...
  return patientStore.examenRes.paciente_examenes.filter(
    (exam) => exam.examenes.pendiente && exam.examenes.pendiente.length > 0
  );
});
                ...
                >
                  <div class="q-pa-sm column" style="gap: 10px">
                    ...
                    <q-list bordered separator dense style="font-size: smaller">
                      <q-item
                        v-for="(result, i) in examen.examenes.validado"
                        :key="i"
                      >
                        <q-item-section>
                          <q-item-label>{{
                            result.examenes.nombre
                          }}</q-item-label>
                        </q-item-section>
                        <q-item-section side>
                          <q-chip
                    ...
                    <div class="row items-center justify-center q-gutter-sm">
                      <q-btn
                        v-if="
                          !expandedGroups.has(i) && getRemainingCount(i) > 0
                        "
                        color="secondary"
                      ...
                      <q-btn
                        ...
                        v-on:click="
                          patientStore.downloadFile(examen.paciente_cod)
                        "
              ...
              <template v-if="pendingExams.length > 0">
                ...
                >
                  <div class="q-pa-sm column" style="gap: 10px">
                    ...
                    <q-list bordered separator dense style="font-size: smaller">
                      <q-item
                        v-for="(result, i) in examen.examenes.pendiente"
                        :key="i"
                      >
                        <q-item-section>
